...
public class DM_U4 implements PlugInFilter {
			...
			for (int y=0; y<height; y++)
				for (int x=0; x<width; x++, pos++)
				{
					...
					if (methode == 2) {
						...
						int r = weicheBlende(z, length, rA, rB);
						int g = weicheBlende(z, length, gA, gB);
						int b = weicheBlende(z, length, bA, bB);
					...
					//TODO CHROMA-KEYING **************************************************
					if (methode == 6) {
						//TrenngraphGraph = y= x +10  -> dr¸ber ist transparent, drunter ist sichtbar . im CbCr Farbraum
						...
						if(YCbCr[1]+30 > YCbCr[2]){
							r=rA;
							g=gA;
							b=bA;
							//‹bergangszone, zweiter Graph
						}else if(YCbCr[1]+30> YCbCr[2]&&YCbCr[1]+40 < YCbCr[2]){
							r=rA/2+rB/2;
							g=gA/2+gB/2;
							b=bA/2+bB/2;
	...
	private int overlay(int a, int b) {
		int r;
		if (a <= 128)  r = a * b / 128;
		else r = 255 - (255 - a) * (255 - b) / 128;
		return r;
	}

	private int weicheBlende(int z, int length, int rA, int rB) {
		return rA  * (z - 1) / (length-1) + rB * (length - z) / (length-1);
	}
